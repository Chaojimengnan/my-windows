...
namespace window {
	...
	constexpr std::pair<int, int> rect_to_width_height(const RECT& rect)
	{
	...
	bool client_size_to_window_size(RECT& rect, DWORD style = 0, bool has_menu = false, DWORD exstyle = 0)
	{
	...
	/// <returns>返回对应的窗口句柄(操作失败返回NULL)</returns>
	HWND find_child_window(const std::string& class_name = "",
	...
	/// <returns>操作是否成功</returns>
	bool get_window_type(HWND window_handle, std::string& class_name)
	{
		CHAR temp_str[MW_MAX_TEXT] = { 0 };
	...
	/// <returns></returns>
	bool set_window_text(HWND window_handle, const std::string& text)
	{
	...
	/// <returns>返回之前的扩展样式，如果操作失败返回0(有点小问题，具体看官方文档)</returns>
	LONG_PTR set_window_attribute(HWND window_handle, int index, LONG_PTR new_attribute)
	{
	...
	LONG_PTR get_window_attribute(HWND window_handle, int index)
	{
	...
	bool is_window_unicode(HWND window_handle)
	{
	...
	bool is_window_handle_vaild(HWND window_handle)
	{
	...
	HWND get_window_from_screen_point(int point_x, int point_y)
	{
	...
	HWND get_child_window_from_parent_point(HWND parent_window, int point_x,
	...
	/// 
	/// 例如：我想只想定义一个`WM_PAINT`(绘制消息)处理函数，可以这么写:
	/// ```cpp
